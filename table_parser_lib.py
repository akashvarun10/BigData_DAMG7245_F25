...
lattice_tables = camelot.read_pdf('./foo.pdf', pages='all', flavor='lattice', suppress_stdout=False)

# Stream -> analyzes text positioning and spaces between text, when structure is maintained through spacing
stream_tables = camelot.read_pdf('./edge_tol.pdf', pages='all', flavor='stream', suppress_stdout=False)
